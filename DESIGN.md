...
- **Recording HUD:** a fixed 520×56 native utility window at the active display's bottom center. It contains a red state dot, `Recording`, elapsed time, `Stop`, `Ask Arco`, and `Invite Arco`. It never steals focus when it appears.
...
- HUD is 520×56. Text and voice use matched, filled 130×34 controls with speech-bubble and waveform icons; Stop uses recording red on a pale red background. Only active recording displays the recording dot; saved state has a check.
...
Text questions and voice participation are independent peer actions, and can remain open together. The meeting header exposes both with matched 150×36 filled controls; the docked text panel owns only text conversation. The HUD exposes the same pair at 130×34 and keeps Stop in its separate red treatment. Ready actions use strong neutral ink with white labels; open text and joined voice use the brand color. Joining retains a progress indicator, joined voice opens participant status, and failed voice offers Invite again. Leaving stays inside the participant surface. The elapsed clock remains isolated from the HUD action subtree.
