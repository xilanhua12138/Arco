...
Hovering or focusing either Arco icon reveals its label to the right of a fixed 36pt icon slot. Revealed labels remain visible while the pointer or keyboard focus stays within the HUD; all labels collapse 160ms after both leave. This preserves target positions when moving between controls. Voice reserves the widest localized state label for the current reveal, preventing connection transitions from shifting adjacent controls. The native panel grows from its existing left edge with the same 160ms ease-out, clamps to the display edge when needed, and restores its original position after collapse. Reduce Motion makes resizing immediate. Capture end and window dismissal clear reveal state.
