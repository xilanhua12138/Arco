...
- The standalone Arco brand wordmark uses bundled Syne Bold at its existing size (24pt sidebar, 17pt onboarding, 16pt provider setup). All other headings, labels, body text and AppKit editors retain the macOS system font. The Syne font and its OFL license ship inside the native UI resource bundle.
...
- **Recording HUD:** a fixed 328×52 native utility window at the active display's bottom center. It contains a red state dot, `Recording`, elapsed time, an icon-only Stop, and separate text-question and voice-participation icons with native tooltips. It never steals focus when it appears.
...
- HUD is 328×52. Text and voice use 36×36 icon controls with speech-bubble and person-with-voice icons; Stop uses recording red on a pale red background. Only active recording displays the recording dot; saved state has a check.
...
## Peer Arco entry points (2026-09-15)

Text questions and voice participation are independent peer actions, and can remain open together. The meeting header exposes both with content-sized 36pt-high controls, dark labels, and a subtle neutral translucent fill; below 1080pt window width, both peer entries switch together to icons; the docked text panel owns only text conversation. The 328×52 HUD uses separate 36×36 icons with native descriptive tooltips, a persistent recording label and timer, and a red Stop before a divider. Idle controls retain a faint neutral fill; hover deepens the same rounded surface. The stop icon keeps recording red without a separate pink tile. Joined voice retains the same person-with-voice symbol with a check badge and pale blue background; open text uses a filled speech bubble and the same pale blue treatment, synchronized with the actual window visibility. Joining retains a progress indicator and failed voice shows an amber retry badge. Joined voice opens participant status; leaving stays inside that surface. Accessible labels retain full action/state names even where visible labels are hidden. The elapsed clock remains isolated from the HUD action subtree.

## HUD label reveal

Hovering either Arco icon reveals its label to the right of a fixed 36pt icon slot. The complete expanded capsule, including its text, remains clickable and hoverable. Leaving that button immediately starts its 160ms collapse; the HUD background and neighboring controls do not retain another button's label. Keyboard focus reveals a label until blur, while focus acquired by clicking does not retain it after pointer exit. Voice reserves the widest localized state label for the current reveal, preventing connection transitions from shifting adjacent controls. SwiftUI animates the content and its outer glass mask together. The native panel reserves space immediately before expansion and releases spare space after the 160ms collapse, without animating its frame. Hosting-view automatic sizing is disabled for the HUD so it cannot override the coordinator's window bounds; the visible capsule stays left-aligned within the temporary canvas. Repeated capture snapshots preserve the visible HUD's size and position. Reduce Motion makes resizing immediate. Capture end and window dismissal clear reveal state.

References: [Material Design expanding action labels](https://m2.material.io/components/buttons-floating-action-button) and [W3C hoverable revealed content](https://www.w3.org/WAI/WCAG21/Understanding/content-on-hover-or-focus). These support button-scoped disclosure and keeping revealed text reachable; animation timing and HUD geometry are Arco-specific choices.
