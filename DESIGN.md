...
Hovering either Arco icon reveals its label to the right of a fixed 36pt icon slot. The complete expanded capsule, including its text, remains clickable and hoverable. Leaving that button immediately starts its 160ms collapse; the HUD background and neighboring controls do not retain another button's label. Keyboard focus reveals a label until blur, while focus acquired by clicking does not retain it after pointer exit. Voice reserves the widest localized state label for the current reveal, preventing connection transitions from shifting adjacent controls. The native panel grows from its existing left edge with the same 160ms ease-out, clamps to the display edge when needed, and restores its original position after collapse. Reduce Motion makes resizing immediate. Capture end and window dismissal clear reveal state.
