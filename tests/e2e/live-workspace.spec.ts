...
test('first launch can choose transcript-only on-device setup without a cloud key', async ({ page }) => {
  ...
  await setup.getByRole('radio', { name: 'Speaker separation model' }).click()
  ...
  expect(persisted.transcription).toContain('"diarization":{"provider":"local","model":"sortformer-streaming"}')
...
test('Settings uses navigable sections in one centered sheet', async ({ page }) => {
  ...
  expect(settingsAlpha === undefined ? 1 : Number(settingsAlpha)).toBeGreaterThanOrEqual(0.96)
  expect(settingsMaterial.backdrop).toBe('none')
  await page.getByRole('button', { name: 'Audio & speakers' }).click()
  const audioSettings = page.getByRole('dialog', { name: 'Audio & speakers' })
  const recognition = audioSettings.locator('summary').filter({ hasText: /^Recognition/ })
  await expect(recognition).toBeVisible()
  await expect(recognition).toContainText('Deepgram · Chinese')
  await expect(audioSettings.getByRole('group', { name: 'Speech recognition location' })).toBeVisible()
  await expect(audioSettings.getByRole('group', { name: 'Speech recognition provider' })).toBeVisible()
  await expect(audioSettings.getByRole('group', { name: 'Speaker separation location' })).toBeVisible()
  await expect(audioSettings.getByRole('group', { name: 'Speaker separation provider' })).toBeVisible()
  await expect(audioSettings.getByText('Remote 1… · In room 1…')).toHaveCount(0)
  await expect(audioSettings.getByText('Speaker labels')).toHaveCount(0)
  await expect(audioSettings.getByText('System audio')).toHaveCount(0)
  await expect(audioSettings.getByText('Room microphone')).toHaveCount(0)
  await expect(audioSettings.getByText(/Labels describe where a voice came from/i)).toHaveCount(0)
  await page.screenshot({ path: 'test-results/arco-product-audio-modes.png', fullPage: true })
  await expect(page.getByText('Deepgram streaming').first()).toBeVisible()
...
test('Recognition configures ASR and streaming diarization providers independently', async ({ page }) => {
  await page.setViewportSize({ width: 1080, height: 750 })
  await gotoConfigured(page, '/?demo=1')
  await page.getByRole('button', { name: 'Stop listening' }).click()
  await expect(page.getByRole('button', { name: 'Back to History' })).toBeVisible()
  ...
  await expect(recognitionSummary).toBeVisible()
  ...
  await expect(settings.getByText('Deepgram streaming', { exact: true }).first()).toBeVisible()
  await expect(settings.getByRole('radio', { name: /Deepgram.*no local model/i })).toBeChecked()
  await expect(settings.getByRole('radio', { name: /LS-EEND Meeting/i })).toHaveCount(0)
  await expect(settings.getByRole('radio', { name: /LS-EEND General/i })).toHaveCount(0)
  ...
  await expect(settings.getByText('ElevenLabs ASR')).toBeVisible()
  await expect(settings.getByText(/separately selected streaming provider/i)).toBeVisible()
  await expect(settings.getByRole('radio', { name: /Deepgram.*no local model/i })).toBeChecked()
  await expect(settings.getByText(/when listening stops|final speaker/i)).toHaveCount(0)
  await expect(recognitionSummary).toContainText('ElevenLabs · Chinese · Deepgram streaming')
  await page.screenshot({ path: 'test-results/arco-elevenlabs-setup.png', fullPage: true })

  const asrLocation = settings.getByRole('group', { name: 'Speech recognition location' })
  await asrLocation.getByText('This Mac', { exact: true }).click()
  await expect(recognitionSummary).toContainText('Nemotron Speech 3.5 not downloaded')
  await expect(recognitionSummary).toContainText('Deepgram streaming')
  await expect(settings.getByRole('group', { name: 'Speech recognition provider' })).toHaveCount(0)
  const diarizationLocation = settings.getByRole('group', { name: 'Speaker separation location' })
  await diarizationLocation.getByText('This Mac', { exact: true }).click()
  const localDiarization = settings.getByRole('group', { name: 'Local speaker separation model' })
  await localDiarization.getByRole('radio', { name: /Streaming Sortformer/i }).click()
  ...
  await expect(settings.getByRole('radio', { name: /Streaming Sortformer/i }).locator('..').getByText('Ready', { exact: true })).toBeVisible()
  ...
  const reopenedAsrLocation = reopened.getByRole('group', { name: 'Speech recognition location' })
  await expect(reopenedAsrLocation.getByRole('radio', { name: /This Mac/i })).toBeChecked()
  await expect(reopened.getByRole('group', { name: 'Speech recognition provider' })).toHaveCount(0)
