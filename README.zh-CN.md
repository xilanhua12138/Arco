...
在历史记录中播放保存的录音，点击词语或句子时间跳转，拖动波形定位转录稿。播放时高亮并跟随当前内容，支持 1× / 1.5× / 2× 倍速。旧记录按已有句级时间定位；没有保存的词级时间不会自动补齐。详见[录音与转录稿联动](docs/features/recording-playback.md)。

## 邀请 Arco 入会

> **v0.3.25 已提供。** 以下为应用真实截图：使用合成语音进行演示会议，展示实时转写、实际生成的回答和已连接的参会悬浮窗。

邀请 Arco 和你一起听会，支持线上、线下和混合会议。系统音频接收远端发言，物理麦克风拾取现场声音。Arco 入会后默认安静，点名发起对话后可以自然追问，不必每句话重复名字。由语音模型根据会议提示词和上下文判断何时回应，应用不再按本地唤醒词或计时窗口静音回复；入会时提供近期会议上下文，需要核对最新信息时直接读取当前转写，不再等待 Codex 先生成一遍答案。
